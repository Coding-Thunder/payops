 ...
 *
 * Client files and links exist as models, services and REST routes
 * (src/server/services/client-file.service.ts, src/app/api/files,
 * src/app/api/links) but nothing in src/app/app or src/components calls them,
 * and /app/customers/[id] renders only "Timeline" and "Orders" tabs. There is
 * no approvals model or route at all — "Approve" appears only as a
 * configurable workflow-transition label (workflow.model.ts:80).
 *
 * The visible copy still makes these claims; that is a product/marketing
 * decision recorded here rather than silently amplified into structured data,
 * where a mismatch is both a rich-result violation and a promise to a visitor
 * arriving from search.
 */
export const FAQ_WITHHELD_FROM_SCHEMA: readonly string[] = [
  "Can I store contracts and files?",
