...
import { CreateCruiseOrderForm } from "./create-cruise-order-form";
 ...
 * page branches before it, so a deployment selling only one thing reaches
 * its form directly with no tab strip, no wrapper element and no extra
 * spacing — exactly the tree it rendered before this component existed.
 *
 * Two decisions worth stating outright:
 *
 *  1. EACH TAB MOUNTS ITS OWN FORM, and every panel is `forceMount`ed so a
 *     half-filled flight request survives a peek at the cruise tab. Radix's
...
/** Tab order. Flights first because it is the higher-volume product;
 *  filtered to what the organization actually sells. */
 ...
 *
 * This is a UI convenience only. The server re-checks the supplier against
 * the order's service type in `buildProviderSnapshotFromKey`, which is what
 * actually holds.
 */
...
interface ServiceTabsProps {
  ...
  /** The active provider catalog, narrowed per tab below. */
  providers: ProviderDTO[];
}
...
export function ServiceTabs({
  ...
  return (
    ...
    >
      ...
      {tabs.includes(ServiceType.CRUISE) ? (
        <TabsContent {...panelProps(ServiceType.CRUISE, active)}>
          <CreateCruiseOrderForm
            {...shared}
            providers={providersForService(providers, ServiceType.CRUISE)}
            active={active === ServiceType.CRUISE}
          />
        </TabsContent>
      ) : null}
