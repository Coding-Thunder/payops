 ...
 * shared union-typed `useForm` across the service tabs: RHF resolves field
 * paths structurally, and a `useForm<CarRental | Flight | Cruise>` makes
 * `trip.pickupDate` and `flight.origin` siblings in the same field registry
 * — which is how half-typed values leak across tabs. One schema, one
 * resolver, one form state per tab.
 *
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Each must travel with an adult.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <ChargeLinesFieldset
              ...
              disabled={isSubmitting}
              namePlaceholder="e.g. Airfare"
              dueLaterLabel={ServiceDueLabel[ServiceType.FLIGHT]}
              totalLabel={ServiceTotalLabel[ServiceType.FLIGHT]}
            />
