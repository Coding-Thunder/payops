 ...
 * the two independent — if it is stale, wrong, or forged, the worst case
 * is that the request is treated as having no selection.
 *
...
/**
 * Read the selection, or null.
 *
 * Returns null rather than throwing when there is no request store at all —
 * a script, the outbox drainer, a background job. Those callers have no
 * cookie by definition and the resolver treats "no selection" as its own
 * case; making this throw would turn every one of them into a 500.
 */
