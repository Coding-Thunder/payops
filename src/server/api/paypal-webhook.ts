...
/**
 * The PayPal webhook implementation, shared by the deployment-level endpoint
 * and the per-organization ones.
 *
 * The TENANT IS SUPPLIED BY THE CALLER rather than resolved here. PayPal
 * verification is a round trip to PayPal using that organization's own
 * `webhookId` and client credentials, so — exactly as with Stripe — the
 * tenant must be known before the payload can be trusted, and the only place
 * it can come from is the endpoint that received the delivery.
 *
...
export async function handlePayPalWebhook(
  req: NextRequest,
  /** Organization whose endpoint received this delivery. */
  organizationId: string,
  /** Whether that organization is the compatibility anchor. Decides whether
   *  unattributed pre-migration orders may be settled by this endpoint. */
