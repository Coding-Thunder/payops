...
export interface PaymentConfirmationEmailProps {
  ...
  /**
   * CAR_RENTAL payload. Null on a FLIGHT / HOTEL order, where it only
   ...
   * `serviceDetailRows()` for FLIGHT / HOTEL. When omitted — which is what
 ...
 * email's, so swapping it in would reword every RentalConfirmation and
 * Trip Reservations receipt.
 *
 * Exported so the payment-request and payment-authorized templates share
 * the one definition — three copies of these literals is three chances for
 * the incumbents' emails to drift apart.
 */
