...
import {
  ...
  listEmails,
  EMAIL_STATUSES,
} from "@/console/server/services/email-ops";
...
export default async function EmailsPage({
  ...
  const hrefForPage = (pg: number) =>
    `${ADMIN_BASE}/emails?${qs({ page: String(pg), pageSize: String(result.pageSize) })}`;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <a
          href={`${ADMIN_API}/emails/export?${qs({})}`}
      ...
      <div className="overflow-x-auto rounded-xl border border-[var(--border)] bg-[var(--panel)]">
        <DataTable caption="Outbound email queue">
          <thead>
            <tr>
              <Th>Kind</Th>
              <Th>Recipient</Th>
            ...
            {result.items.length === 0 ? (
              ...
              result.items.map((e) => (
                <tr key={e.id} className="hover:bg-white/[0.02]">
                  ...
                  <Td>
                    <Link
                      href={`${ADMIN_BASE}/emails/${e.id}`} prefetch={false}
