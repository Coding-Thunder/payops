...
import {
  ...
  ORDER_STATUSES,
  GATEWAYS,
} from "@/console/server/services/orders";
import { parsePagination } from "@/console/server/pagination";
import { Badge, Pagination, StatTile, Td, Th, fmtDateTime, DataTable } from "@/console/components/ui";
...
export default async function OrdersPage({
  ...
  const hrefForPage = (pg: number) =>
    `${ADMIN_BASE}/orders?${qs({ page: String(pg), pageSize: String(result.pageSize) })}`;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <a
          href={`${ADMIN_API}/orders/export?${qs({})}`}
      ...
      >
        ...
        <Link href={`${ADMIN_BASE}/orders`} className="rounded-lg px-3 py-1.5 text-sm text-[var(--muted)] hover:text-slate-200">
          Reset
        </Link>
      </form>

      <div className="overflow-x-auto rounded-xl border border-[var(--border)] bg-[var(--panel)]">
        <DataTable caption="Orders and payments">
          <thead>
            <tr>
              <Th>Order</Th>
              <Th>Customer</Th>
            ...
            {result.items.length === 0 ? (
              ...
              result.items.map((o) => (
                <tr key={o.id} className="hover:bg-white/[0.02]">
                  <Td>
                    <Link href={`${ADMIN_BASE}/orders/${o.id}`} prefetch={false} className="font-mono text-[12px] text-sky-300 hover:underline">
                      {o.orderNumber}
                    </Link>
                  </Td>
                  <Td>
                  ...
                  <Td>
                    <Link href={`${ADMIN_BASE}/orders/${o.id}`} prefetch={false} className="text-sky-300 hover:underline">
                      View
                    </Link>
                  </Td>
                </tr>
