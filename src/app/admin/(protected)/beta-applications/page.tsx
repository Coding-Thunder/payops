...
import { listBetaApplications } from "@/console/server/services/beta-applications";
...
export default async function BetaApplicationsPage({
  ...
  const hrefForPage = (pg: number) => {
    ...
    return `${ADMIN_BASE}/beta-applications?${params.toString()}`;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-xl border border-[var(--border)] bg-[var(--panel)]">
        <DataTable caption="Beta applications">
          <thead>
            <tr>
              <Th>Name</Th>
              <Th>Email</Th>
            ...
            {result.items.length === 0 ? (
              ...
              result.items.map((a) => (
                <tr key={a.id}>
                  <Td>
                    <Link
                      href={`${ADMIN_BASE}/beta-applications/${a.id}`} prefetch={false}
