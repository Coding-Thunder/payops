...
import { Organization, User, Types } from "@/console/server/db/models";
import {
  buildPageResult,
  type Pagination,
  type PageResult,
...
export async function getUserDetail(id: string): Promise<UserDetail | null> {
  ...
  if (u.primaryOrgId) {
    const o = await Organization.findById(u.primaryOrgId)
      .select({ name: 1 })
      .lean<{ name?: string } | null>();
    orgName = o?.name ?? null;
  }
  const ownsOrg = await Organization.exists({ ownerUserId: oid });
  ...
  return {
    ...
    status: u.status ?? "—",
    orgName,
    orgId: u.primaryOrgId ? String(u.primaryOrgId) : null,
    isOrgOwner: Boolean(ownsOrg),
    lastLoginAt: u.lastLoginAt ? new Date(u.lastLoginAt).toISOString() : null,
    createdAt: u.createdAt ? new Date(u.createdAt).toISOString() : null,
    hasPassword: Boolean(u.passwordHash),
    firebaseLinked: Boolean(u.externalAuth?.firebaseUid),
  };
}
