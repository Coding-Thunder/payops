...
import { Organization, OrgMember, Quotation, User, Types } from "@/console/server/db/models";
import { generateResetToken, buildSetPasswordUrl } from "@/console/server/auth/reset-token";
import { sendAccessLinkEmail } from "@/console/server/email/mailer";
import { recordAdminAction } from "@/console/server/audit";
import { ORG_SLUG_REGEX } from "@/server/db/models/organization.model";
import { RESERVED_SLUGS } from "@/server/services/signup.service";
...
// The console provisions organizations, so it must apply exactly the same
// slug rule the main app's signup applies. Both come from their canonical
// homes rather than a copy with a "keep in sync" comment.
...
export async function grantAccessToWaitlist(
  ...
  //
  // No re-hash loop is needed: the canonical token binds to
  // `sha256(hash).base64url.slice(0,16)`, and base64url's alphabet contains
  // no ".", so the payload's 4-part split can never be broken by a bcrypt
  // salt that happens to start with a dot.
  ...
  const token = generateResetToken({ _id: userId!, passwordHash });
