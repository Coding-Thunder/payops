 ...
 *
 * ── Registration names are prefixed `Console` ────────────────────────────
 * Mongoose keeps ONE model registry per process. Nine of the mirrors below
 * (User, Organization, OrgMember, Quotation, PendingEmail, AuditLog, Order,
 * Customer, BetaApplication) share a name with a main-app model. When the
 * console ran as its own Next app that was harmless — separate processes.
 * In one process it is not, and it does not fail loudly: neither this
 * helper nor the main app's `registerModel()` calls bare
 * `mongoose.model(name, schema)` on an already-registered name, so instead
 * of `OverwriteModelError` you get a SILENT, load-order-dependent schema
 * hijack. Whichever module a given route bundle evaluates first wins, and
 * the loser gets the other's schema: either the main app loses every
 * validator, enum, default and hook, or the console loses the `strict:false`
 * escape hatch its extra fields (`Quotation.grantedAt`, …) depend on.
 *
 * Binding each mirror to a distinct registration name removes the shared
 * key entirely, so there is nothing left for load order to decide. The
 * `collection:` option — already explicit on every schema below — is what
 * points the model at the right collection, and Mongoose fully supports
 * several models over one collection. The exported const names are
 * unchanged, so no call site cares.
 *
 * `Console`-prefixed names are used ONLY for the nine shared ones; the four
 * console-owned models (AdminOtp / AdminUser / AdminNote / AdminAudit) have
 * no main-app counterpart and keep their names.
 *
...
export const User = model<UserDoc>("ConsoleUser", userSchema);
...
export const OrgMember = model<OrgMemberDoc>("ConsoleOrgMember", orgMemberSchema);
...
export const Quotation = model<QuotationDoc>("ConsoleQuotation", quotationSchema);
...
export const AuditLog = model<AuditLogDoc>("ConsoleAuditLog", auditLogSchema);
...
export const Order = model<OrderDoc>("ConsoleOrder", orderSchema);
...
export const Customer = model<CustomerDoc>("ConsoleCustomer", customerSchema);
...
//
// NOTE: the app now runs on the main connection, which pins `autoIndex:false`
// in production, so NONE of the `schema.index(...)` calls in this file are
// built automatically there. They are mirrored in
// `scripts/ensure-prod-indexes.ts` (`ensureConsoleIndexes`) — add any new one
// in BOTH places and run `npm run ensure-indexes:prod`.
...
// The admin allow-list. This collection is the ONLY source of who can obtain
// an admin session (see `isAllowedEmail`) — there is no environment-level
// bootstrap. An empty or unreachable collection therefore locks everyone out
// by design; recovery is a direct insert here. Admin-owned + strict.
