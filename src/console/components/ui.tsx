...
export function Th({
  ...
  children: React.ReactNode;
  /** Right-align to match a numeric <Td>. */
  numeric?: boolean;
}) {
  return (
    <th
      scope="col"
      className={`border-b border-[var(--border)] px-3 py-2 text-[11px] font-medium uppercase tracking-wider text-[var(--muted)] ${
...
export function Td({
  ...
  children: React.ReactNode;
  /**
   * Money/counts. Right-aligns and switches to tabular figures so digits line
   * up column-wise — the difference between scanning a column of amounts and
   * reading every row. Deliberately NOT for IDs or dates-as-text.
   */
  numeric?: boolean;
}) {
  return (
    <td
      className={`border-b border-[var(--border)]/60 px-3 py-2 text-sm text-slate-200 ${
...
/**
 * A table with an accessible name. Screen readers announce the caption, and
 * without one a bare <table> in a page full of panels is just "table".
 */
