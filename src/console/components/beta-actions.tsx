    ...
    confirm(
      {
        ...
        body: "This emails the applicant a single-use activation link that expires in 7 days. Approving again later will not send a second invitation.",
        confirmLabel: "Approve & send",
      },
      async () => {
        await call("approve", `${ADMIN_API}/beta-applications/${id}/approve`);
      },
    );
  const reject = () =>
    confirm(
      {
        ...
        body: "They will not get access, and this cannot be undone from the console.",
        confirmLabel: "Reject application",
        tone: "danger",
      },
      async () => {
        await call("reject", `${ADMIN_API}/beta-applications/${id}/reject`, {
          note: note.trim() || undefined,
        });
      },
    );
  const saveNote = async () => {
    const res = await call("note", `${ADMIN_API}/beta-applications/${id}/note`, {
      note,
    });
