...
import { googleSignInIdToken, isFirebaseConfigured } from "@/console/lib/firebase-client";
...
export function LoginForm({ next }: { next?: string | null }) {
  ...
  async function requestEmailOtp(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const r = await postJson(`${ADMIN_API}/auth/request-otp`, { email });
  ...
  async function signInGoogle() {
    ...
    try {
      ...
      const r = await postJson(`${ADMIN_API}/auth/google`, { idToken });
  ...
  async function verify(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const r = await postJson(`${ADMIN_API}/auth/verify-otp`, { email, code });
      if (!r.ok) throw new Error(r.error?.message ?? "Invalid code");
      // `next` is validated server-side before it reaches this component.
      window.location.href = next ?? `${ADMIN_BASE}/dashboard`;
    } catch (err) {
