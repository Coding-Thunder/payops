...
import { Pagination, Td, Th, fmtDateTime } from "@/components/ui";
...
export default async function AuditPage({
  ...
  const hrefForPage = (pg: number) =>
    `/audit?${qs({ page: String(pg), pageSize: String(result.pageSize) })}`;
            ...
            {result.items.length === 0 ? (
              ...
              result.items.map((a) => (
                <tr key={a.id} className="hover:bg-white/[0.02]">
                  ...
                  <Td>
                    <Link
                      href={`/audit/${a.id}?source=${a.source}`}
