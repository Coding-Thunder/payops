  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <Link href="/orders" className="text-[12px] text-[var(--muted)] hover:text-slate-200">
            ← Orders &amp; Payments
          </Link>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
            ...
            <ul className="divide-y divide-[var(--border)]/60">
              {o.emails.map((e) => (
                <li key={e.id} className="flex items-center justify-between gap-3 py-2">
                  <Link href={`/emails/${e.id}`} className="text-[13px] text-sky-300 hover:underline">
                    {e.kind}
                  </Link>
        ...
        <Card title={`Audit (${o.audit.length})`}>
          <div className="mb-2 text-right">
            <Link
              href={`/audit?source=product&id=${o.id}`}
            ...
            <ul className="divide-y divide-[var(--border)]/60">
              {o.audit.map((a) => (
                <li key={a.id} className="flex items-center justify-between gap-3 py-2">
                  <Link
                    href={`/audit/${a.id}?source=product`}
